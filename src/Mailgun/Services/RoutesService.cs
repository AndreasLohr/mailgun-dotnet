...
internal sealed class RoutesService : IRoutesService
{
    ...
    public async Task<Route> GetAsync(string id, CancellationToken cancellationToken = default)
    {
        ...
        var env = await _http.GetJsonAsync<RouteSingleEnvelope>($"v3/routes/{PathEscape.Segment(id)}", null, cancellationToken).ConfigureAwait(false);
        return env.Route;
    }
    ...
    public async Task<Route> CreateAsync(CreateRouteRequest request, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var a in request.Actions)
            fb.Add("action", a);
        var env = await _http.PostFormAsync<RouteSingleEnvelope>("v3/routes", fb, cancellationToken).ConfigureAwait(false);
        return env.Route;
    }
    ...
    public async Task<Route> UpdateAsync(string id, UpdateRouteRequest request, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var a in request.Actions)
            fb.Add("action", a);
        var env = await _http.PutFormAsync<RouteSingleEnvelope>($"v3/routes/{PathEscape.Segment(id)}", fb, cancellationToken).ConfigureAwait(false);
        return env.Route;
    }
